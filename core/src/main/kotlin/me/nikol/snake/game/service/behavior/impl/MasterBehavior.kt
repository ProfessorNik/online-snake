...
import me.nikol.snake.game.config.GameFactory
import me.nikol.snake.game.exceptions.IllegalDirectionException
import me.nikol.snake.game.exceptions.PlayerNotFoundException
import me.nikol.snake.game.exceptions.SnakeNotFoundException
...
class MasterBehavior(
    ...
    override var playerId: Int = GameFactory.GAME_CREATOR_ID
) : Behavior {
    ...
    override var messageFormer = MessageFormer(playerId)

    override var gameConfig: GameConfig = gameFactory.gameConfig()
    private var gameState: GameState = gameFactory.gameState()
    private val joinPlayerDelayedMessages: MutableList<DelayedMessage<GameMessage<JoinMsg>>> = mutableListOf()
            ...
            if (System.currentTimeMillis() - entry.value
                ...
                gameState.players.players.find { it.id == entry.key }?.let { player ->
                    if (player.ipAddress != null && player.port != null) {
                        ...
                        onRemovePlayers.putIfAbsent(playerId, System.currentTimeMillis())
                    }
                }
            }
        }
