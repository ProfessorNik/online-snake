...
import ru.fithub.snakes.core.exceptions.CantAddException
import java.util.*
...
data class GameState(
    ...
    private fun deletedCrashedSnakes(gameConfig: GameConfig): GameState {
        val addedFood = mutableListOf<Coord>()
        return copy(snakes = snakes.fold(listOf()) { survivorsSnakes, snake ->
            if (snake.crashed(snakes, gameConfig)) {
                snake.alignPoints(gameConfig).forEach{ if(random(0,1) == 1) addedFood.add(it) }
                survivorsSnakes
            } else {
                survivorsSnakes + snake
            }
        }, foods = foods + addedFood)
    }
    ...
    private fun countPoints(): GameState {
        return copy(
            players =
            players.copy(
    ...
    private fun countPointsForCrashed(player: GamePlayer, gameConfig: GameConfig): GamePlayer {
        val playerSnake = findSnakeByGamePlayer(player)
        if (playerSnake != null) {
            return player.copy(score = player.score + snakesWithout(playerSnake).fold(0) { acc, snake ->
    ...
    private fun snakesWithout(missing: Snake): List<Snake> {
    ...
    fun findPlayerById(playerId: Int): GamePlayer? {
        return players.players.find { it.id == playerId }
    }

    fun findPlayerByName(name: String): GamePlayer? {
        return players.players.find { it.name == name }
    }

    fun findSnakeByGamePlayerId(playerId: Int): Snake? {
        return snakes.find { it.playerId == playerId }
    }
                    ...
                    if (x in 2 until gameConfig.width - 2) {
                        if (y in 2 until gameConfig.height - 2) {
                            if (!hasSomethingAround(x, y, gameConfig)) {
                                ...
                                coords.addAll(countCoordsForSnake(x, y, direction))
                                return@br
                            }
                        }
                    }
    ...
    fun makeDeputy(receiverIp: String, receiverPort: Int): GameState {
        return copy(players = GamePlayers(players.players.map {
            if (it.ipAddress == receiverIp && it.port == receiverPort){
                it.copy(role = NodeRole.DEPUTY)
            } else {
                it
            }
        }))
    ...
    fun containsDeputy(): Boolean {
        return players.players.find { it.role == NodeRole.DEPUTY } != null
    }
...
data class Snake(
    ...
    fun isLegalNewDirection(newDirection: Direction): Boolean {
        return when (direction) {
...
fun coordsForFirstSnake(gameConfig: GameConfig): List<Coord> {
    val coords = mutableListOf<Coord>()
    val x = (1..gameConfig.width - 2).random()
    val y = (1..gameConfig.height - 2).random()
    val direction = Direction.getByValue((1..4).random())
    coords.addAll(countCoordsForSnake(x, y, direction))
    return coords
}

private fun countCoordsForSnake(
    x: Int,
    y: Int,
    direction: Direction
): MutableList<Coord> {
    val coords = mutableListOf(Coord(x, y))
    when (direction) {
